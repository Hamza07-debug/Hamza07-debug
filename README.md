<div align="center">

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:1e1b4b,50:4c1d95,100:6d28d9&height=220&section=header&text=Muhammad%20Hamza%20Afzaal&fontSize=42&fontColor=E9D5FF&animation=fadeIn&fontAlignY=38&desc=AI%20Engineering%20Intern%20%C2%B7%20CS%20Student%20%40%20FAST-NUCES&descAlignY=58&descSize=18" />

<br/>

[![Typing SVG](https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=500&size=15&duration=3000&pause=1000&color=A78BFA&center=true&vCenter=true&width=650&lines=Building+Agentic+AI+systems+with+Claude%2C+MCP+%26+n8n;AI+Engineering+Intern+%7C+CS+Student;BS+Computer+Science+%40+FAST-NUCES;Open+to+internships+and+collaboration)](https://git.io/typing-svg)

<br/>

![Academic](https://img.shields.io/badge/BS_Computer_Science-FAST--NUCES-1e1b4b?style=for-the-badge&labelColor=2e1065&color=7c3aed)
![Location](https://img.shields.io/badge/Location-Lahore%2C_Pakistan-1e1b4b?style=for-the-badge&labelColor=2e1065&color=7c3aed)

[![LinkedIn](https://img.shields.io/badge/LinkedIn-1e1b4b?style=for-the-badge&logo=linkedin&logoColor=A78BFA&labelColor=2e1065)](https://www.linkedin.com/in/hamza-afzaal-80400b415/)
[![Email](https://img.shields.io/badge/Email-1e1b4b?style=for-the-badge&logo=gmail&logoColor=A78BFA&labelColor=2e1065)](mailto:your.email@example.com)
[![GitHub](https://img.shields.io/badge/GitHub-1e1b4b?style=for-the-badge&logo=github&logoColor=A78BFA&labelColor=2e1065)](https://github.com/Hamza07-debug)
[![X](https://img.shields.io/badge/X-1e1b4b?style=for-the-badge&logo=x&logoColor=A78BFA&labelColor=2e1065)](https://x.com/hamzaafzaal07)

<br/>

![Profile Views](https://komarev.com/ghpvc/?username=Hamza07-debug&style=for-the-badge&color=7c3aed&labelColor=1e1b4b)
![Followers](https://img.shields.io/github/followers/Hamza07-debug?style=for-the-badge&labelColor=1e1b4b&color=7c3aed)
![Stars](https://img.shields.io/github/stars/Hamza07-debug?style=for-the-badge&labelColor=1e1b4b&color=7c3aed)

</div>

<br/>

## 🧬 About Me

> A Software Engineer in progress, engineering AI systems that reason, plan, and act.

I'm a Computer Science student at **FAST-NUCES (Chiniot-Faisalabad Campus)**, currently working as an **AI Engineering Intern**, where I build agentic AI workflows using **Claude API**, the **Model Context Protocol (MCP)**, and **n8n**. My work sits at the intersection of strong CS fundamentals — data structures, operating systems, database systems — and modern applied AI: LLM orchestration, retrieval-augmented generation, and multi-agent tooling.

I care about writing production-grade, well-documented systems rather than one-off scripts — every project I ship gets a clean architecture, a sanitized public repo, and a README that explains *why*, not just *what*.

**🎯 Open To:** AI Engineering Internships &nbsp;·&nbsp; Agentic AI / LLM Projects &nbsp;·&nbsp; Backend & Systems Collaboration &nbsp;·&nbsp; Remote or Lahore/On-site

<br/>

---

## 🧠 Tech Stack

**Languages**

![C](https://img.shields.io/badge/C-1e1b4b?style=flat-square&logo=c&logoColor=A78BFA)
![C++](https://img.shields.io/badge/C++-1e1b4b?style=flat-square&logo=cplusplus&logoColor=A78BFA)
![Python](https://img.shields.io/badge/Python-1e1b4b?style=flat-square&logo=python&logoColor=A78BFA)
![JavaScript](https://img.shields.io/badge/JavaScript-1e1b4b?style=flat-square&logo=javascript&logoColor=A78BFA)
![SQL](https://img.shields.io/badge/SQL-1e1b4b?style=flat-square&logo=postgresql&logoColor=A78BFA)
![x86 Assembly](https://img.shields.io/badge/x86_Assembly-1e1b4b?style=flat-square&logo=nasm&logoColor=A78BFA)

**Frontend**

![HTML5](https://img.shields.io/badge/HTML5-1e1b4b?style=flat-square&logo=html5&logoColor=A78BFA)
![CSS3](https://img.shields.io/badge/CSS3-1e1b4b?style=flat-square&logo=css3&logoColor=A78BFA)
![Tkinter](https://img.shields.io/badge/Tkinter-1e1b4b?style=flat-square&logo=python&logoColor=A78BFA)

**Backend & Databases**

![FastAPI](https://img.shields.io/badge/FastAPI-1e1b4b?style=flat-square&logo=fastapi&logoColor=A78BFA)
![Oracle](https://img.shields.io/badge/Oracle-1e1b4b?style=flat-square&logo=oracle&logoColor=A78BFA)
![MongoDB](https://img.shields.io/badge/MongoDB-1e1b4b?style=flat-square&logo=mongodb&logoColor=A78BFA)
![PostgreSQL](https://img.shields.io/badge/PL%2FSQL-1e1b4b?style=flat-square&logo=postgresql&logoColor=A78BFA)

**AI, Cloud & Tooling**

![Claude](https://img.shields.io/badge/Claude_API-1e1b4b?style=flat-square&logo=anthropic&logoColor=A78BFA)
![n8n](https://img.shields.io/badge/n8n-1e1b4b?style=flat-square&logo=n8n&logoColor=A78BFA)
![Git](https://img.shields.io/badge/Git-1e1b4b?style=flat-square&logo=git&logoColor=A78BFA)
![GitHub](https://img.shields.io/badge/GitHub-1e1b4b?style=flat-square&logo=github&logoColor=A78BFA)
![Linux](https://img.shields.io/badge/Linux-1e1b4b?style=flat-square&logo=linux&logoColor=A78BFA)
![DOSBox](https://img.shields.io/badge/DOSBox-1e1b4b?style=flat-square&logo=freebsd&logoColor=A78BFA)

<br/>

---

## 🤖 AI / ML Expertise

| Domain | Proficiency | Details |
|---|---|---|
| Agentic AI | ⭐⭐⭐⭐ | Claude API, Agent SDK, Model Context Protocol (MCP), n8n workflow automation |
| LLM Applications | ⭐⭐⭐⭐ | RAG pipelines, QLoRA fine-tuning fundamentals, prompt engineering, OpenAI/Gemini/Groq APIs |
| Search Algorithms | ⭐⭐⭐⭐ | BFS, DFS, UCS, A*, RBFS, Hill Climbing, Simulated Annealing, Minimax, Alpha-Beta Pruning |
| Machine Learning | ⭐⭐⭐ | K-Means / K-Medoids Clustering, Perceptrons, ANNs, Backpropagation |
| Logic & CSP | ⭐⭐⭐ | Propositional Logic, First-Order Logic, Constraint Satisfaction (Sudoku, N-Queens), Wumpus World |

<br/>

---

## 🚀 Featured Projects

<details>
<summary><b>🎓 University Management System (DSA)</b></summary>
<br/>

A console-based University Management System implemented in C++ using custom data structures, with authentication and campus navigation.

| | |
|---|---|
| **Stack** | C++ |
| **Data Structures** | Hash Tables, Graphs, AVL Trees, Stacks, Queues |
| **Features** | User authentication, campus navigation, core academic-record management |
| **Impact** | Demonstrates applied DSA outside of textbook problems |
| **Repository** | [University-Management-System-DSA](https://github.com/Hamza07-debug/University-Management-System-DSA) |

</details>

<details>
<summary><b>👾 Atari Breakout — x86 Assembly</b></summary>
<br/>

A fully functional Atari Breakout arcade game built in x86 Assembly using BIOS & DOS interrupts, running as a 16-bit DOS `.COM` file in DOSBox.

| | |
|---|---|
| **Stack** | NASM, x86 Assembly, DOSBox |
| **Core Concepts** | Direct memory/video buffer manipulation, BIOS/DOS interrupt handling, low-level game loop |
| **Impact** | Built for the Computer Organization & Assembly Language (COAL) course |
| **Repository** | [Atari-Breakout-x86-Assembly](https://github.com/Hamza07-debug/Atari-Breakout-x86-Assembly) |

</details>

<details>
<summary><b>⚔️ Fantasy Arena</b></summary>
<br/>

A modular turn-based battle game built with core OOP principles.

| | |
|---|---|
| **Stack** | C++ |
| **Core Concepts** | Inheritance, polymorphism, composition, aggregation, operator overloading |
| **Impact** | Applied OOP design patterns in a full game-state system |
| **Repository** | [Fantasy-Arena](https://github.com/Hamza07-debug/Fantasy-Arena) |

</details>

<details>
<summary><b>🗡️ Adventure Quest</b></summary>
<br/>

A two-player strategic adventure game with persistent game state.

| | |
|---|---|
| **Stack** | C++ |
| **Core Concepts** | Inheritance, polymorphism, dynamic memory management, file handling, game-state persistence |
| **Impact** | End-to-end OOP system design with save/load functionality |
| **Repository** | [Adventure-Quest](https://github.com/Hamza07-debug/Adventure-Quest) |

</details>

<details>
<summary><b>🧩 Nexus Wumpus Intelligence</b></summary>
<br/>

A knowledge-based agent that navigates the Wumpus World using logical inference.

| | |
|---|---|
| **Stack** | Vanilla JavaScript |
| **Core Concepts** | Propositional Logic Knowledge Base, Resolution Refutation |
| **Impact** | Applied AI logic/reasoning course concepts to a playable browser agent |
| **Repository** | [nexus-wumpus-intelligence](https://github.com/Hamza07-debug/nexus-wumpus-intelligence) |

</details>

<details>
<summary><b>⚽ FC24 Analytics Pro</b></summary>
<br/>

Advanced performance metrics and predictive modeling for the FC24 professional player population.

| | |
|---|---|
| **Stack** | JavaScript, Probability & Statistics |
| **Impact** | Applied statistical modeling to a real sports-data domain |
| **Repository** | [fc24-analytics-pro](https://github.com/Hamza07-debug/fc24-analytics-pro) |

</details>

<br/>

---

## 💼 Experience

**AI Engineering Intern**
*Current*

Building agentic AI systems and automation pipelines using modern LLM tooling.

- Designed and implemented agentic workflows using **n8n**, the **Claude API**, and **MCP (Model Context Protocol)**
- Integrated Claude-based agents into automation pipelines for real-world task execution

`Claude API` `MCP` `n8n` `Agentic AI` `Python`

<br/>

---

## 🏆 Achievements

<div align="center">

| Recognition | Details |
|---|---|
| 6 Featured Repositories | Spanning x86 Assembly, C++ OOP, DSA, and AI/logic-based systems |
| 31 Repository Stars | Across 17 public repositories |
| Cross-Domain Project Range | Low-level systems (Assembly) to applied AI (Wumpus World agent) |

</div>

<br/>

---

## 📊 GitHub Analytics

<div align="center">

<img src="https://github-stats-extended.vercel.app/api?username=Hamza07-debug&show_icons=true&rank_icon=github&theme=dark&hide_border=true&bg_color=1e1b4b&title_color=A78BFA&icon_color=A78BFA&text_color=c4b5fd" width="49%" />
<img src="https://github-stats-extended.vercel.app/api/top-langs/?username=Hamza07-debug&layout=compact&theme=dark&hide_border=true&bg_color=1e1b4b&title_color=A78BFA&text_color=c4b5fd" width="30%" />

<br/>

<img src="https://streak-stats.demolab.com?user=Hamza07-debug&theme=dark&hide_border=true&background=1e1b4b&ring=7c3aed&fire=A78BFA&currStreakLabel=A78BFA&sideLabels=c4b5fd&dates=c4b5fd" width="60%" />

</div>

<br/>

## 🏅 GitHub Trophies

<div align="center">

<img src="https://github-profile-trophy.vercel.app/?username=Hamza07-debug&theme=darkhub&no-frame=true&margin-w=10&column=7" />

</div>

<br/>

## 📈 Contribution Activity

<div align="center">

<img src="https://github-readme-activity-graph.vercel.app/graph?username=Hamza07-debug&theme=react-dark&bg_color=1e1b4b&color=A78BFA&line=7c3aed&point=c4b5fd&hide_border=true" width="90%" />

</div>

<br/>

## 🐍 Contribution Snake

<div align="center">

<img src="https://raw.githubusercontent.com/Hamza07-debug/Hamza07-debug/output/github-contribution-grid-snake-dark.svg" width="90%" />

</div>

<br/>

---

## 🎯 Current Focus

```yaml
learning:
  - Large Language Models (LLMs) & RAG pipelines
  - QLoRA fine-tuning
  - Agent SDK & advanced MCP tooling
building:
  - Agentic AI workflows with n8n + Claude API
  - Portfolio-grade DSA & systems projects
exploring:
  - Multi-agent orchestration patterns
  - Production-grade prompt architecture
open_to:
  - AI Engineering Internships
  - Remote / Lahore-based collaboration
```

<br/>

---

## 📫 Connect

[![Email](https://img.shields.io/badge/Gmail-1e1b4b?style=for-the-badge&logo=gmail&logoColor=A78BFA&labelColor=2e1065)](mailto:your.email@example.com)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-1e1b4b?style=for-the-badge&logo=linkedin&logoColor=A78BFA&labelColor=2e1065)](https://www.linkedin.com/in/hamza-afzaal-80400b415/)
[![GitHub](https://img.shields.io/badge/GitHub-1e1b4b?style=for-the-badge&logo=github&logoColor=A78BFA&labelColor=2e1065)](https://github.com/Hamza07-debug)
[![X](https://img.shields.io/badge/X-1e1b4b?style=for-the-badge&logo=x&logoColor=A78BFA&labelColor=2e1065)](https://x.com/hamzaafzaal07)

<br/>

<div align="center">

*"Success is the result of continuous learning, consistent effort, and persistence."*

<br/>

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:6d28d9,50:4c1d95,100:1e1b4b&height=120&section=footer" />

</div>
